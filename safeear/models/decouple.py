...
class SpeechTokenizer(nn.Module):
    ...
    def forward(self, 
        ...
        # 确保 quantized_list[1:] 不为空
        if len(quantized_list) > 1:
            acoustic_tokens = torch.stack(quantized_list[1:])
        else:
            acoustic_tokens = torch.zeros((0, x.shape[1], self.dimension), device=x.device)

        return o, commit_loss, feature, acoustic_tokens
